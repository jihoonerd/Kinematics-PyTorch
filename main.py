...
def learnable_kinematics():
    ...
    final_prediction = simple_dnet(motion_data)
    custom_model = CustomModel(kinematic_chain, final_prediction, channel_name=channel_name)
    robot_model = RobotModel(custom_model)
    robot_model.set_frame(0)
    robot_model.forward_kinematics(robot_model.kinematic_model.root_name)
    final_chain = robot_model.kinematic_chain
    print(final_prediction.detach().numpy())

if __name__ == "__main__":
    learnable_kinematics()