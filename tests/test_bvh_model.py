...
def test_get_kinematic_chain(kinematic_model):
...
def test_get_root_pos_rot(kinematic_model):
    bvh_model = kinematic_model
    pos0, rot0 = bvh_model.get_root_pos_rot(frame_id=0, channel_order='XYZ')
    ...
    pos9, rot9 = bvh_model.get_root_pos_rot(frame_id=9, channel_order='XYZ') # TODO: after visualization try zyx xyz
    np.testing.assert_almost_equal(pos9.numpy(), np.array([[9.3598, 17.8506, -17.3603]]).T, decimal=6)

    z_rot = Rotation.from_euler('z', -3.9143).as_matrix()
    y_rot = Rotation.from_euler('y', -7.0800).as_matrix()
    x_rot = Rotation.from_euler('x', -2.0917).as_matrix()
    rot_ref = x_rot @ y_rot @ z_rot
